...
result_human = []
result_human_text = []
with open(r'data/TolokaPersonaChat_gk_1_500_withoutgender.jsonl',
...
# features
# dialog replicas
npro, nounroot, obl, xcomp,\
    csubj, nmod, noun_case, adj_noun,\
    adj, prtf, pobj = ([] for i in range(14))
verb_plus_inf, verb_constr,\
    npro_descripive = ([] for i in range(3))
verb_plus_obj, inf_plus_obj,\
    prtf_noun, digit_noun,\
    numr, cc_conj = ([] for i in range(6))

# persona description
npro_p, nounroot_p, \
    obl_p, xcomp_p, \
    csubj_p, nmod_p, \
    noun_case_p, adj_noun_p, \
    adj_p, prtf_p, pobj_p = ([] for i in range(11))
verb_plus_inf_p, verb_constr_p, \
    npro_descripive_p = ([] for i in range(3))
verb_plus_obj_p, inf_plus_obj_p, \
    prtf_noun_p, digit_noun_p, \
    numr_p, cc_conj_p = ([] for i in range(6))
            ...
            for token in doc:
                ...
                for i in range(len(syntax)):
                    ...
                    try:
                        ...
                        word_next = [letter for letter in word_next if letter.isalpha() and letter not in alp]
            ...
            for p in persona_:
                ...
                for token in doc_p:
                    ...
                    for word2, nextWord2 in zip(syntax, syntax[1:]):
                        ...
                        elif (word_parse.tag.POS == "VERB" and word_parse.tag.person == "1per"
                              and word_parse.tag.number == "sing") and (word_parse2.tag.POS != "PREP"
                                                                        or word_parse2.tag.POS != "CONJ" or
                                                                        word_parse2.tag.POS == "ADVB"):
                            ...
                            digit_noun_p.append(word_parse2.normal_form)
                              ...
                              and word_parse.tag.number == "sing") \
                                and (word_parse2.tag.POS == "NOUN"
                                     or word_parse2.tag.POS == "ADJF"
                                     or word_parse2.tag.POS == "PRTF"
                                     or word_parse2.tag.POS == "PRTS"):
                            npro_descripive_p.append(word_parse2.normal_form)

                        elif sintagma == "cc" \
                                and sintagma_next == "conj":
