...
class TestData:
    ...
    def test_collator(self):
        ...
        assert len(data) == 5
        assert len(target_ids[0]) == answer_maxlength and isinstance(target_ids, torch.Tensor)
        assert len(passage_ids[0][0]) == text_maxlength and isinstance(passage_ids, torch.Tensor)
