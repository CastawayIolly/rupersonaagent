...
from speech_extraction.SpeechExtraction import \
    read_dataset, get_sentence_and_words,\
    get_pos_tags, get_speech_characteristic
...
class TestSpeechExtraction:
    def test_read_dataset(self):
        dataset_value = read_dataset('test_name',
                                     'test_dataset.csv', 'RowId', ',')
    ...
    def test_get_sentence_and_words(self):
        ...
        assert_that(get_sentence_and_words(test_data),
                    equal_to((1, 3, 4.0, 0, 1)))

    def test_get_pos_tags(self):
        test_data = pd.Series({'text': 'I like cookies'})
        assert_that(get_pos_tags(test_data, 1),
                    equal_to({'NNS': 2.0, 'IN': 1.0}))
